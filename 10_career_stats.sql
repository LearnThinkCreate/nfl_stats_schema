...
-- Aggregates from player_season_stats to create career-level statistics
...
SELECT
    ...
    SUM(passing_yards) - SUM(passing_air_yards) AS passing_yards_after_catch,
    ...
    -- QB targets (for ADOT calculation)
    ...
    SUM(team_season_targets) AS team_career_targets,
    SUM(team_season_air_yards) AS team_career_air_yards
FROM
    player_season_stats
GROUP BY
    player_id,
    season_type;  -- Group by season_type to keep REG and POST separate

-- Add derived metrics as a separate step
CREATE OR REPLACE VIEW player_career_stats_with_metrics AS
SELECT 
    pc.*,
    
    -- Per-game averages
    ROUND(completions::numeric / NULLIF(games_played, 0), 1) AS completions_per_game,
    ROUND(attempts::numeric / NULLIF(games_played, 0), 1) AS attempts_per_game,
    ROUND(passing_yards::numeric / NULLIF(games_played, 0), 1) AS passing_yards_per_game,
    ROUND(carries::numeric / NULLIF(games_played, 0), 1) AS carries_per_game,
    ROUND(rushing_yards::numeric / NULLIF(games_played, 0), 1) AS rushing_yards_per_game,
    ROUND(targets::numeric / NULLIF(games_played, 0), 1) AS targets_per_game,
    ROUND(receptions::numeric / NULLIF(games_played, 0), 1) AS receptions_per_game,
    ROUND(receiving_yards::numeric / NULLIF(games_played, 0), 1) AS receiving_yards_per_game,
    ...
    CASE 
        WHEN (attempts + sacks) > 0 THEN ROUND((sacks::numeric / (attempts + sacks)) * 100, 1)
        ELSE NULL
    END AS sack_rate,
    ...
    CASE 
        WHEN team_career_targets > 0 THEN ROUND((targets::numeric / team_career_targets), 3)
        ELSE NULL
    END AS target_share,

    CASE WHEN 
        team_career_air_yards > 0 THEN ROUND((receiving_air_yards::numeric / team_career_air_yards), 3)
         ELSE NULL 
    END AS air_yards_share,

    -- Weighted Opportunity Rating (WOPR)
    CASE 
        WHEN team_career_targets > 0 AND team_career_air_yards > 0 THEN 
            ROUND((1.5 * (targets::numeric / team_career_targets)) + 
                  (0.7 * (receiving_air_yards::numeric / team_career_air_yards)), 3)
        ELSE NULL
    END AS wopr,
...
FROM
    player_career_stats pc;

-- Comment on the views
COMMENT ON VIEW player_career_stats IS 
'Career-level player statistics, separated by regular season and postseason.
Includes games_played count and all raw statistics.
Use player_career_stats_with_metrics for advanced metrics and derived stats.
Converted from materialized view to regular view for optimization.';

COMMENT ON VIEW player_career_stats_with_metrics IS
'Complete player career statistics with all derived metrics.
Includes all the advanced metrics at the career level.
